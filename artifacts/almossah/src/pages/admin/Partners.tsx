          ...
          <Dialog open={isOpen} onOpenChange={setIsOpen}>
            ...
            <DialogContent className="sm:max-w-md font-sans" dir="rtl">
              ...
              <form onSubmit={handleSubmit} className="space-y-4 mt-4">
                ...
                <div>
                  <label className="block text-sm font-medium mb-1">الشعار</label>
                  <div className="flex items-center gap-3">
                    <label className="cursor-pointer flex items-center gap-2 bg-slate-100 hover:bg-slate-200 border border-slate-300 rounded-lg px-4 py-2 text-sm font-medium transition-colors">
                      <ImageIcon size={16} />
                      اختر من المعرض
                    ...
                    {formData.logoUrl && (
                      <div className="relative">
                        <img src={formData.logoUrl} alt="" className="h-14 w-20 object-contain rounded-lg border border-slate-200 bg-white p-1" />
                        <button type="button" onClick={() => setFormData(prev => ({ ...prev, logoUrl: "" }))} className="absolute -top-1 -right-1 bg-red-500 text-white rounded-full w-4 h-4 flex items-center justify-center text-xs leading-none">×</button>
                      </div>
                    )}
                  </div>
                </div>
